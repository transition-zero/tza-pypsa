...
def export_to_excel(
    ...
    storage_hourly = network.storage_units_t.p
    prices_hourly = network.buses_t.marginal_price
    generation_monthly = network.generators_t.p.resample('ME').sum()
    storage_monthly = network.storage_units_t.p.resample('ME').sum()
    ...
    with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
        ...
        storage_hourly.to_excel(writer, sheet_name='Storage by node (hr)')
        prices_hourly.to_excel(writer, sheet_name='Prices by node (hr)')
        storage_monthly.to_excel(writer, sheet_name='Storage by node (m)')
